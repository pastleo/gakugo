defmodule Gakugo.Db do
  ...
  def change_unit(%{} = unit, attrs) do
    %Unit{
      id: Map.get(unit, :id) || Map.get(unit, "id"),
      title: Map.get(unit, :title) || Map.get(unit, "title"),
      from_target_lang: Map.get(unit, :from_target_lang) || Map.get(unit, "from_target_lang"),
      deleted_at: Map.get(unit, :deleted_at) || Map.get(unit, "deleted_at")
    }
    |> Unit.changeset(attrs)
  end

  def change_unit(%Unit{} = unit), do: change_unit(unit, %{})

  def change_unit(%{} = unit), do: change_unit(unit, %{})
